...
public class ReduceSideJoinReducer extends Reducer<Text, Text, Text, Text> {
    ...
    private void union(Context context) throws IOException, InterruptedException {
        if (!leftSide.isEmpty()){
            context.getCounter(GROUP_NAME, UNION).increment(1);
            for (Text l : leftSide){
                if (!rightSide.isEmpty()){
                    context.getCounter(GROUP_NAME, UNION).increment(1);
                    out.write(empty, l, UNION);
                    for (Text r : rightSide){
                        out.write(empty, r, UNION);
                    }
                } else {
                    out.write(empty, l, UNION);
                }
